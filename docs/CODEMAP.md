...
│   ├── server.ts          # entry point: Express + WebSocketServer
│   ├── pty-manager.ts     # PiInstance lifecycle: spawn, kill, attach, resize
│   ├── session-api.ts     # Express router for /api/* endpoints
│   ├── session-store.ts   # Scan ~/.pi/agent/sessions/ for history
│   └── config.ts          # JSON config persistence (token, port, agentDir)
├── public/                # Static frontend files
│   ├── index.html         # Layout: sidebar, tabs, terminals, login
│   ├── app.js             # All frontend logic (~270 lines)
...
| `spawnPi(cwd)` | `pty-manager.ts` | Spawn pi in PTY, return PiInstance |
| `spawnPiWithSession(path)` | `pty-manager.ts` | Spawn pi with `--session <path>` |
| `attachWebSocket(id, ws)` | `pty-manager.ts` | Subscribe WS to PTY output |
| `listInstances()` | `pty-manager.ts` | Return active {id, cwd, pid, createdAt} |
| `listSessions()` | `session-store.ts` | Scan disk, parse headers, return history |
...
| `attachInstance(id, cwd)` | `app.js` | Create WS, xterm.js, pane, tab |
